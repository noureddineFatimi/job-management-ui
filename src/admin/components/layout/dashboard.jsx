...
import { BriefcaseIcon, AdjustmentsHorizontalIcon, UserCircleIcon, CodeBracketSquareIcon, EyeIcon , Cog6ToothIcon, TrashIcon, PlusIcon} from "@heroicons/react/24/outline";
...
const items = [
  getItem("Gestion des offres d'emploi", "sub1", <BriefcaseIcon style={{ width: "16px" }} />, [
    ...
    getItem("Publier une offre", "/dashboard/offres/add", <PlusIcon style={{ width: "16px" }} />),
  ]),
...
{contextHolder}
  ...
  scrollbarGutter: 'stable',}}
        >
        <div style={{padding:"1rem", marginBottom:"1rem", marginTop:"1rem",marginRight:"1rem",marginLeft:"1rem",display:"flex", justifyContent:"center", alignItems:"center", background:colorBgContainer, borderRadius:"8px"}} > <img src={websiteLogo} style={{height:"30px"}} alt="logo" /> </div> 
      ...
      { loading ? <div style={{width:"100%",display
        ...
        <Header style={{ alignItems: "center",
            width: "100%", background: colorBgContainer, gap:"1rem",display:"flex", justifyContent:"end",fontWeight:"100", fontSize:"20px" }} > <div> Bonjour {' '}<span style={{fontWeight:"bold"}}> {user && user.first_name && user.first_name.toUpperCase()} </span> {' '} dans votre espace! </div> <Button danger type='default' onClick={handleDisconnect}>Déconnexion</Button> </Header>
